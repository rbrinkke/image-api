...
# ============================================================================
# Validates HS256 JWT tokens issued by Auth API's OAuth Authorization Server
# Uses shared secret for simple, fast, and secure token validation


class JWTAuthMiddleware(BaseHTTPMiddleware):
    """OAuth 2.0 JWT validation middleware with HS256.

    Validates JWT tokens using shared secret (HS256 algorithm).
    This is the recommended approach for internal microservices.

    Architecture:
        - Auth API issues HS256 tokens signed with JWT_SECRET_KEY
        - Image API validates tokens using the same shared secret
        - No remote API calls needed - stateless validation
        - All claims (user_id, scopes, org_id) embedded in token

    Flow:
        1. Extract Bearer token from Authorization header
        2. Validate token signature using shared secret
        3. Verify token expiry, issuer, audience
        4. Store validated payload in request.state
        5. Endpoints check permissions via dependencies

    Security:
        - JWT_SECRET_KEY must be securely shared between services
        - Tokens are stateless - no session storage needed
        - Supports token expiry, audience validation, scope checking
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            logger.info("jwt_expired", token_prefix=token[:20] if token else "")
            ...
            return self._unauthorized_response(f"Invalid token claims: {e}")
